...
主笔每轮必须重新读取脚本，以下事实只描述当前 `v6.31` 脚本。

### 4.1 中转脚本事实

当前中转脚本头部写明：

- `install_transit_v6.31.sh`
- 架构：CN2 GIA 纯 IPv4 中转机。
- Nginx stream SNI 盲传。
- 禁止代理核心和 IPv6 业务路径。
- 当前版本说明：同步版本号；本轮业务修复在落地脚本，重启后防火墙持久化恢复。
...
- `install_landing_v6.31.sh`
- 架构：美国落地机。
- Xray-core 4 协议单端口回落。
- Cloudflare DNS-01 证书。
- 禁止 IPv6 业务路径。
- 当前版本说明：修复落地机重启后防火墙持久化脚本动态规则残留模板占位符导致恢复失败的问题。
...
当前基线是 `v6.31`。下一轮真实优化应统一为 `v6.32`。默认真实修复要涨版本。

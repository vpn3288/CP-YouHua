...
主笔每轮必须重新读取脚本，以下事实只描述当前 `v6.25` 脚本。

### 4.1 中转脚本事实

当前中转脚本头部写明：

- `install_transit_v6.25.sh`
- 架构：CN2 GIA 纯 IPv4 中转机。
- Nginx stream SNI 盲传。
- 禁止代理核心和 IPv6 业务路径。
- 当前版本说明：禁用 Debian Nginx 默认站点，避免中转机额外监听 80 端口。
...
- `install_landing_v6.25.sh`
- 架构：美国落地机。
- Xray-core 4 协议单端口回落。
- Cloudflare DNS-01 证书。
- 禁止 IPv6 业务路径。
- 当前版本说明：同步中转禁用 Nginx 默认站点修复版本；落地业务逻辑不变。
...
当前基线是 `v6.25`。下一轮真实优化应统一为 `v6.26`。默认真实修复要涨版本。

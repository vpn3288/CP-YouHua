...
主笔每轮必须重新读取脚本，以下事实只描述当前 `v6.28` 脚本。
...
- Nginx stream SNI 盲传。
- 禁止代理核心和 IPv6 业务路径。
- 当前版本说明：修复 cron 环境 PATH 缺少 sbin 导致健康检查误报防火墙丢失。
...
- 禁止 IPv6 业务路径。
- 当前版本说明：同步健康检查 cron PATH 修复版本；落地业务逻辑不变。
...
当前基线是 `v6.28`。下一轮真实优化应统一为 `v6.29`。默认真实修复要涨版本。

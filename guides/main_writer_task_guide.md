...
主笔每轮必须重新读取脚本，以下事实只描述当前 `v6.30` 脚本。

### 4.1 中转脚本事实

当前中转脚本头部写明：

- `install_transit_v6.30.sh`
- 架构：CN2 GIA 纯 IPv4 中转机。
- Nginx stream SNI 盲传。
- 禁止代理核心和 IPv6 业务路径。
- 当前版本说明：修复中转机已有落地记录后新增第二台/第三台时查重无匹配触发 pipefail 退出的问题。
...
- `install_landing_v6.30.sh`
- 架构：美国落地机。
- Xray-core 4 协议单端口回落。
- Cloudflare DNS-01 证书。
- 禁止 IPv6 业务路径。
- 当前版本说明：同步版本号；本轮业务修复在中转脚本，多落地导入查重 no-match 退出。
...
当前基线是 `v6.30`。下一轮真实优化应统一为 `v6.31`。默认真实修复要涨版本。

...
- 当前远端提交（本轮 v6.08 未推送前）: `61b41dc fix: harden transit route recovery v6.05`
...
主笔每轮必须重新读取脚本，以下事实只描述当前 `v6.08` 脚本。
...
- Nginx stream SNI 盲传。
- 禁止代理核心和 IPv6 业务路径。
- 当前版本说明：stream include 校验必须同时包含 marker 和真实 include 行。
...
当前基线是 `v6.08`。下一轮真实优化应统一为 `v6.09`。默认真实修复要涨版本。
